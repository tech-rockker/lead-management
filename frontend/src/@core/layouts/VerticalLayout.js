...
import { loadWordAll } from '../../utility/apis/redWord'
import { useRedux } from '../../redux/useRedux'
import { useTranslation } from 'react-i18next'
import { loadPackageDetails } from '../../utility/apis/commons'
...
const VerticalLayout = (props) => {
  ...
  useEffect(() => {
    if (isValid(routerProps?.meta?.title)) {
      document.title = `${FM(routerProps?.meta?.title)} - ${
        user?.name ? decrypt(user?.name) : 'Aceuss'
      }`
    }
  }, [routerProps, location, user])
  ...
  useEffect(() => {
    setIsMounted(true)
    if (isValid(user?.id)) {
      if (!isValidArray(paragraphs?.data)) {
        loadParagraph({
          perPage: 10000,
          dispatch
        })
