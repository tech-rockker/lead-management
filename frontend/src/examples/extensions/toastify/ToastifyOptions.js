...
const Toastr = () => {
  const notifyDelay = () =>
    toast.error(<ErrorToast />, { icon: false, autoClose: 10000, hideProgressBar: true })
  const notifySticky = () =>
    toast.info(<InfoToast />, { icon: false, autoClose: false, hideProgressBar: true })
  const notifyAutoClose = () => {
    return toast.success(<AutoCloseToast />, {
      icon: false,
      autoClose: 3000,
