...
import { HourglassEmptyOutlined, MenuOpen, ViewWeek, WarningOutlined } from '@material-ui/icons'
...
import { CompanyTypes, empType, ShiftType, UserTypes } from '../../../../../utility/Const'
...
import { deleteMessageSelected } from '../../../../chat/store'
...
const ScheduleFormTab = ({
  ...
  const loadIpOption = async (search, loadedOptions, { page }) => {
    if (isValid(watch('patient_id'))) {
      const res = await loadPatientPlanList({
        ...
        page,
        perPage: 100,
        jsonData: { name: search, status: 1, user_id: watch('patient_id') }
      })
          ...
          success: (d) => {
            ...
            for (const key in d) {
              p[key] = {
                ...d[key],
                remaining_hours:
                  d[key]?.patient_assigned_hours -
                  /* d[key]?.patient_planned_hours +*/ d[key]?.patient_completed_hours
              }
            }
            setAddedPatients({
              ...addedPatients,
          ...
          success: (d) => {
            ...
            if (isValidArray(d)) {
              ...
              if (isValidArray(dSet)) {
                ...
                dataSet.push({
                  start_date: new Date(empView?.joining_date),
                  end_date: new Date(addDay(new Date(empView?.joining_date), 27)),
          ...
          if (isValid(sets)) {
            ...
            if (isValid(found) && user === found?.emp_id) {
              ...
              const nextShift2 = previousShift2
                ...
                : null
              // const _7thDay = getFirstAndLast(schedule?.date)?.lastDay
              const _7thDay = addDay(schedule?.date, 6)

              if (hourLimit?.hasOwnProperty(user)) {
                hourLimit[user] = hourLimit[user] + (diff.minutesTotal - restDiff.minutesTotal)
              } else {
                hourLimit[user] = diff.minutesTotal - restDiff.minutesTotal
              }

              if (shift?.shift?.shift_type === ShiftType.Emergency) {
                if (emergencyHours?.hasOwnProperty(user)) {
                  emergencyHours[user] =
                    emergencyHours[user] + (diff.minutesTotal - restDiff.minutesTotal)
                } else {
                  emergencyHours[user] = diff.minutesTotal - restDiff.minutesTotal
                }
              }
              ...
              const remainingMinutes =
                previousRemainingMinutes - (diff.minutesTotal - restDiff.minutesTotal)
              ...
              const remainingMinutesPatient =
                previousRemainingMinutesPatient - (diff.minutesTotal - restDiff.minutesTotal)
              ...
              const total = hourLimit?.hasOwnProperty(user)
                ? hourLimit[user]
  ...
  const sortShifts = (date, shifts, more = false) => {
    ...
    const isInvalidEmp = new Date(empView?.joining_date) > date
      ...
      setAddedEmployees({
        ...
        [employee_id]: {
          ...
          assigned_work: {
            ...emp?.assigned_work,
            actual_working_hours_per_week_in_min: Math.round(
              emp?.assigned_work?.actual_working_hour_per_week * 60
            )
          }
        }
      })
              ...
              <Scrollbars autoHide>
                <div className=''>
                  <Row className='p-1 me-1 gx-1'>
                    ...
                    <Show IF={isValid(empView?.id) && new Date(empView?.joining_date) > new Date()}>
                      <Col md='12'>
      ...
      <Menu id={'menu-employee'}>
        ...
        <Item
          disabled={
            ...
            new Date(empView?.joining_date) > new Date(contextMenuData?.date)
          }
          onClick={() => {
        ...
        >
          ...
          {FM(FM(isValid(empView?.id) ? 'new-shift-with' : 'new-shift'))}{' '}
