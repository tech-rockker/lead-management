...
import { viewSchedule } from '../../../../utility/apis/schedule'
import { FM, log } from '../../../../utility/helpers/common'
import CenteredModal from '../../../components/modal/CenteredModal'
import Shimmer from '../../../components/shimmers/Shimmer'
import ScheduleTabs from './ScheduleTabs'
import EmployeeInfoCard from './Tabs/EmployeeInfoCard'
...
const ScheduleViewModal = ({
  ...
  log('calendareditData', editData)
  return (
    <>
      <CenteredModal
        disableFooter={true}
        title={title ?? `${FM(`schedule-details`)}`}
        modalClass={'modal-lg'}
        open={open}
        scrollControl={false}
              ...
              <Show IF={editData?.user_id === null}>
                <Col xl='5' lg='5' xs={{ order: 0 }} md={{ order: 1, size: 4 }}>
                  <Card className='text-align-center'>
                    <Badge color='primary'>{FM('no-employee')}</Badge>
                  </Card>
                </Col>
              </Show>
              <Col xl='7' lg='7' xs={{ order: 1 }} md={{ order: 0, size: 7 }}>
                <ScheduleTabs user={editData} followup={editData} step={step} />
              </Col>
            </Row>
          </div>
        )}
