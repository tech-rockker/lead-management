...
const SinglePrintDev = ({
  ...
  const handleSave = (data) => {
    printDeviation({
      jsonData: {
        ...data,
        print_with_secret: is_secret === 1 ? 'yes' : 'no',
        patient_id,
        deviation_id
      },
      loading: setLoading,
    ...
    >
      ...
      <Form onSubmit={handleSubmit(handleSave)}>
        <ModalBody>
          <Row>
            {/* <Col md="12">
                            <p className='text-danger text-small-12'>{FM('you-can-leave-dates-blank-to-print-all-journals')}</p>
                        </Col> */}
            {/* <Col md="6">
                            <FormGroupCustom
                                name={"from_date"}
            ...
            {/* <Col md="12">
                            <FormGroupCustom
                                ...
                                onChangeValue={(e) => {
                                    log(e)
                                    setValue('print_with_secret', e === true ? 1 : 0)
                                }}
                                className="mb-2"
                                control={control}
                                rules={{ required: false }}
                                checked={watch('print_with_secret') === 1}
                            />
                        </Col> */}
            <Col md='12'>
              <FormGroupCustom
