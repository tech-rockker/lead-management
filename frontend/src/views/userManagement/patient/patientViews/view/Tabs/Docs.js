...
const DocsTab = ({ user }) => {
  const oldFiles = isValidArray(user?.admin_files) ? user?.admin_files : []
  ...
  const renderOldFilePreview = (file) => {
    if (checkURL(file?.file_path)) {
      return (
        <img className='rounded' alt={file.file_name} src={file?.file_path} height='28' width='28' />
      )
    } else {
      return <FileText url={file?.file_path} size='28' />
    }
  }
